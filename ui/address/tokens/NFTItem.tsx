...
const NFTItem = ({ token, value, isLoading, withTokenLink, chain, ...tokenInstance }: Props) => {
  ...
  const tokenInstanceLink = tokenInstance.id ?
    route({ pathname: '/token/[hash]/instance/[id]', query: { hash: token.address_hash || token.address || '', id: tokenInstance.id } }) :
    undefined;
    ...
    <NFTItemContainer position="relative">
      ...
      <Flex justifyContent="space-between" w="100%" flexWrap="wrap">
        <Flex ml={ 1 } overflow="hidden">
          ...
          <NftEntity hash={ token.address_hash || token.address || '' } id={ tokenInstance.id } isLoading={ isLoading } noIcon/>
        </Flex>
