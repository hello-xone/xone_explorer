      ...
      const tokenId = (() => {
        if (!Array.isArray(data.change)) {
          if ('token_id' in data && data.token_id) {
            return (
              <NftEntity
                hash={ data.token.address_hash || data.token.address || '' }
                id={ data.token_id }
                isLoading={ isLoading }
              />
            );
        ...
        return <TxStateTokenIdList items={ data.change } tokenAddress={ data.token.address_hash || data.token.address || '' } isLoading={ isLoading }/>;
      })();
